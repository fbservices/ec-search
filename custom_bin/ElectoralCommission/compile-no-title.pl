...
my $file = $collection_cfg;
$file =~ s/collection\.cfg/no-title.log/;
...
sub pre_process_file {
    ...
    return if ( not defined $temp );
    return if ( -d $temp );

    # Open file
    if ( not open( DATA, "<", $temp ) ) {
        warn "EC-Compile: Unable to read file $temp ($!)";
        return;
    }
