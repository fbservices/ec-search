...
# Description: Instant index script to add doc-summary data to relevant binary files.
...
if ( ($#ARGV + 1) < 1 ) {
    print "\nEC Pre-Index-Instant: Usage: $0 <collection config file>\n\n";
    exit 1;
}

die("EC Pre-Index-Instant: Unable to open $collection_cfg")
...
die("EC Pre-Index-Instant: Data root doesn't exist: $data_path") if ( not -d $data_path );

# As a precaution, don't let this script be run on anything but a valid
# Funnelback data directory
$data_path =~ s/offline/live/;
$data_path =~ s/live\/data/live\/secondary-data/;
if ( not $data_path =~ /(live|offline)[\\\/]secondary-data/i ) {
    print "ERROR: This script should only be run on a collection's instant data directory.\n$data_path\n";
    exit 1;
}

# Timestamp start
my $date = scalar(localtime);
print "EC Pre-Index-Instant: Processing files at $date...\n";
print "Data path: $data_path\n";

# Print out Welsh pages for gscoping
my $fileGscopeInstant = $collection_cfg;
$fileGscopeInstant =~ s/collection\.cfg/gscope-welsh-instant.cfg/;
# Open file
if ( not open( GSCOPEINSTANT, ">", $fileGscopeInstant ) ) {
    warn "EC Pre-Index-Instant: Unable to write to file $fileGscopeInstant ($!)";
    return;
}
...
print "EC Pre-Index-Instant: Finished processing files at $date\n";
...
sub pre_process_file {
    ...
    if ( -d $file or $file =~ /\.(pdf|doc|xls|ppt|ps|jpg|mp3|mpg|mpeg|zip)\./i ) {
        return;
    }

    # Open file
    if ( not open( DATA, "<", $file ) ) {
        warn "EC Pre-Index-Instant: Unable to read file $file ($!)";
        return;
    }
    ...
    if ($file =~ m/^(?:document-summary)|(?:_nocache)\?assetid=/) {
	...
	if ($content =~ m/<p><a href="(.+?)" target="_blank">Download the document<\/a><\/p>/) {
	    ...
	    if ($doc_lang =~ m/cym/) {
		print GSCOPEINSTANT "3 $site$doc_url\n";
            }
	...
	# Mark any non-binary pages (web pages) for gscoping if the language is 'cym' or 'eng_cym'
	    ...
	    $content =~ s/<\/head>/<meta name="search_section_cymraeg" content="$doc_section_cymraeg" \/>\n<\/head>/;
            ...
            print GSCOPEINSTANT "3 $doc_url\n";
        }
    }

    if ( not open( DATA, ">", $file) ) {
        warn "EC Pre-Index-Instant: Unable to write to file $file ($!)";
        return;
    }
